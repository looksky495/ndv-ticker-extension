...
export type EarthquakeOperator = {
	jma: {
		url_list: string;
		jsonlist: string[];
		tracker_list: any;
		shindo_list: Record<string, number>;
		magnitude_not_a_number: Record<string, number>;
		load: () => Promise<void>;
		vxse5x: (src: string, type: "vxse51" | "vxse52" | "vxse53") => Promise<void>;
	};
	dmdata: Record<string, unknown>;
	events: Record<string, unknown>;
	source: string;
	notice: boolean;
	vxse51: (_data: object) => Promise<void>;
	vxse52: (_data: object) => Promise<void>;
	vxse53: (_data: object) => Promise<void>;
	vxse61: (_data: object) => Promise<void>;
	vxse62: (_data: object) => Promise<void>;
	view: (_id: string) => Promise<void>;
	latestId: number;
	quakeList: any[];
	quakeData: Record<string, any>;
};

export function createEarthquakeOperator(): EarthquakeOperator {
	const operator: EarthquakeOperator = {
		jma: {
			...
			async load (){
				const list = await fetch(this.url_list + "?_=" + Date.now()).then(res => res.json()) as QuakeList.QuakeList;
			...
			async vxse5x (src, type){
				...
				const handler = (operator as any)[type];
