...
export type TsunamiOperator = {
	...
	subscribe: (listener: (state: TsunamiOperator, vtse41?: string, vtse51?: string) => void) => () => void;
	...
	notifyUpdate: (vtse41?: string, vtse51?: string) => void;
};

export function createTsunamiOperator(): TsunamiOperator {
	const listeners = new Set<(state: TsunamiOperator, vtse41?: string, vtse51?: string) => void>();

	const operator: TsunamiOperator = {
		...
		notifyUpdate (vtse41?, vtse51?){
			for (const listener of listeners) listener(this, vtse41, vtse51);
			this.onUpdate?.(this, vtse41, vtse51);
		},
		async load (){
			...
			for (const area of data.Body.Tsunami.Observation.Item){
				for (const item of area.Station){
					if (item.MaxHeight){
						...
						if ("ArrivalTime" in item.FirstHeight){
