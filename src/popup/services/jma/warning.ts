...
import { createAreaOperator, type AreaOperator } from "./common.ts";
...
export function createWarnCurrentOperator(areaOperator: AreaOperator = createAreaOperator()): WarnCurrentOperator {
	const listeners = new Set<(text: string) => void>();

	const operator: WarnCurrentOperator = {
		...
		async load(){
			const AreaData = await areaOperator.getData();
